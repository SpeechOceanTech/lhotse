    ...
    with open(wavscp_file, 'r') as fw, \
        ...
        for line in tqdm(fseg.readlines(), desc="Reading Segments..."):
            try:
                ...
                continue
            if "segments" not in aid2infos[aid]:
                aid2infos.pop(aid)
                continue
            if sid not in sid2text.keys():
                print(sid)
                continue
            sid_info = {}
            sid_info["sid"] = sid
            sid_info["begin_time"] = start
            sid_info["end_time"] = end
...
def prepare_kalditype_speech(
    ...
    done_file = output_dir / f".{subset_name}.done"
    Touch(str(done_file))
    return manifests
...
def parse_utterance(
    ...
    for seg in audio["segments"]:
        if seg["end_time"] > audio["duration"]:
            break
